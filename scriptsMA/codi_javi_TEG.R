...
source<-read.csv2('data_water_sources_adria.csv')
...
source[which(source$author == 'Muñoz-Villers'), 'author'] <- 'Munoz-Villers'
...
multiple<-subset(source,label_class=='soil') %>%    ###this function computes the linear regressions
  nest(-authorYear,-date,-plot) %>%                       ###here you can choose which factors should be used
  mutate(
...
multiple<-multiple[,c('authorYear','date','plot','term','estimate','std.error',"statistic",'p.value')]  ###select only relevant columns
...
rsquared<-rsquared[,c('authorYear','date','plot','r.squared')]  ###select only relevant columns

###count the number of soil samples, in case we want to cut off using this
length<- source %>% count(authorYear,date,plot)
...
swl<-merge(intercept,slope,by=c('authorYear','date','plot'))   ###create a new table with separate columns for intercept and slope

###give proper names
colnames(swl)<-c("authorYear",'date','plot',"term","estimate","std.error","statistic","p.value","term.slope","estimate.slope","std.error.slope",
                 "statistic.slope","p.value.slope")
##paste rsquareds
swl<-merge(swl,rsquared,by=c('authorYear','date','plot'))

#past N's
swl<-merge(swl,length,by=c('authorYear','date','plot'))
...
plant<-merge(plant,swl,by=c('authorYear','date','plot'))   
...
# before doing this, check the species names thoroughly
# figure out what to do when species is "several"
# double check typos, capitals and small case

plant<-merge(plant,meta,by=c('authorYear','species'))  ##attach metadata, I think plot not specified
